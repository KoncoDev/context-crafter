...
export const PROJECT_STATUS_CONFIG: Record<ProjectStatus, StatusConfig> = {
	draft: {
		...
		color: 'text-ink-3 border-rule-2 bg-panel',
		isAIProcessing: false
	},
	extracting_master: {
		label: 'Extracting',
		description: 'AI is building the master source file',
		color: 'text-parchment border-parchment/40 bg-parchment/10',
		isAIProcessing: true
	},
	pending_master_validation: {
		label: 'Review Master',
		description: 'Master source ready for your review',
		color: 'text-ochre border-ochre/40 bg-ochre/10',
		isAIProcessing: false
	},
	generating_personas: {
	...
	processing_variants: {
		...
		color: 'text-bone border-bone/30 bg-bone/8',
		isAIProcessing: false
	},
	completed: {
		label: 'Completed',
		description: 'All variants have been approved',
		color: 'text-moss border-moss/40 bg-moss/10',
		isAIProcessing: false
	},
	error: {
		label: 'Error',
		description: 'An error occurred — requires intervention',
		color: 'text-rust border-rust/40 bg-rust/10',
		isAIProcessing: false
	}
};
